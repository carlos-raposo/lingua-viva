...
async function loadLiveTrends() {
    ...
    // Tentar carregar TODAS AS FONTES EM PARALELO (PT + BR + AO + Bluesky)
    if (typeof fetchAllSources === 'function') {
        console.log("✅ fetchAllSources está disponível - CARREGANDO TUDO");
        
        const trendData = await fetchAllSources();
        
        console.log(`📊 Resultado: ${trendData ? trendData.length : 0} itens`);
        
        if (trendData && trendData.length > 0) {
            // Verificar se tem dados reais (não mockData)
            const hasLiveData = trendData.some(t => t.source === 'google_trends' || t.source === 'bluesky' || t.origem.includes('Google') || t.origem.includes('BLUESKY'));
    ...
    } else if (typeof fetchLiveTrends === 'function') {
        // Fallback para função original se fetchAllSources não existir
        console.log("⚠️  fetchAllSources não disponível, usando fetchLiveTrends");
        ...
        console.error("❌ fetchAllSources e fetchLiveTrends NÃO ESTÃO DISPONÍVEIS!");
...
// Função para atualizar matriz com dados arbitrários
function updateMatrixWithData(trendData) {
    console.log("\n📡 ===== ATUALIZANDO MATRIZ COM NOVOS DADOS =====");
    
    const badge = document.getElementById('data-status-badge');
    
    if (trendData && trendData.length > 0) {
        // Verificar se tem dados reais
        const hasLiveData = trendData.some(t => t.source === 'google_trends' || t.source === 'bluesky');
...
// ==============================================================================
// FAZER FUNÇÃO GLOBAL PARA ATUALIZAR MATRIZ
// ==============================================================================
if (typeof window !== 'undefined') {
    window.updateMatrixData = updateMatrixWithData;
    console.log("✅ updateMatrixData disponível globalmente");
}
